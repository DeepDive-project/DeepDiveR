...
add_model <- function(config = NULL, lstm_nodes = NULL, dense_nodes = NULL, 
  ...
  #### add handling of errors to make sure lstm_nodes and dense_nodes contain
  #### at least one numeric entry, or that if it is a vector it goes largest
  #### number to smallest number.
  lstm_nodes <- as.numeric(lstm_nodes)
  dense_nodes <- as.numeric(dense_nodes)
  
  lstm_parameter_name <- paste0("lstms_layers_model_", model_name)
  config$data$model_training[[lstm_parameter_name]] <- lstm_nodes
  
  dense_parameter_name <- paste0("dense_layers_model_", model_name)
